...
template <typename T>
typename bintree<T>::inorder_iterator & bintree<T>::inorder_iterator::operator--(){
    if(elnodo.null())
    	return *this;
    if(!elnodo.left().null()){
    	elnodo=elnodo.left();
    	...
    	while(!elnodo.parent().null() && elnodo.parent().left()==elnodo)
    		elnodo=elnodo.parent();
...
template<typename T>
void bintree<T>::swap(bintree<T> & x){
  unsigned int aux_nodos=num_nodos;
  typename bintree<T>::node aux_raiz=laraiz;

  num_nodos=x.num_nodos;
  laraiz=x.laraiz;

  x.num_nodos=aux_nodos;
  x.laraiz=aux_raiz;
}

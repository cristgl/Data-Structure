...
using namespace std;
	...
	/** @brief Pasa a tipo de dato booleano el string
	@param[in] str cadena a convertir
	@return true si la cadena es "true", false en caso contrario
	*/
...
int main()
{
    ...
    fecha f;
	 ...
	 conjunto::const_iterator iter;
	 conjunto::description_iterator itd;
    

    load(ChicagoDB, "crimenes.csv");
	itd=ChicagoDB.dbegin("POS");

	while(itd!=ChicagoDB.dend() && cont < 200){
		cout << cont << (*itd)<<"\n";
		cont++;
		itd++;
	}
	...
	cout << "#################################################################################";
	itd--;
	while(itd!=ChicagoDB.dbegin("POSS") && cont < 200){
		cout << cont << (*itd)<<"\n";
		cont++;
		itd--;
	}

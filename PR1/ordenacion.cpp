...
void selection(vector<string> & T,int inicial, int final){
 int inicio, indice_elem_menor,i;
 string elemento_menor,aux;

	for(inicio=inicial; inicio < final; inicio++){
		elemento_menor = T[inicio];
		indice_elem_menor = inicio;
 	 
		for(i=inicio;i<final;i++){
	 		if(elemento_menor > T[i]){
				elemento_menor = T[i]; 
	 			indice_elem_menor = i;
			}
	 	}
	 	...
	 	aux = T[inicio];
	 	T[inicio] = elemento_menor;
	 	T[indice_elem_menor] = aux;

 	}
}
 
void insertion(vector<string> & T, int inicial, int final){
	string aux;
	bool placed=false;

	for(int i=inicial;i<final;i++){
		aux=T[i];
		for (int j=i; j>inicial && !placed ; j--){
			if(T[j-1]<=T[j])
				placed=true;
			else {
				T[j]=T[j-1];
...
////////////////////////////////////////////////////
// ORDENACION POR BURBUJA, SELECION, INSERCION    //
////////////////////////////////////////////////////

	vector<string> aux;
	//cout << "Ordenacion Burbuja"<< endl;
	for (int tama = 100; tama < 85000 ; tama+= 5000){
		aux =Q;
		start = clock();
		burbuja(aux, 0,tama );
		//selection(aux,0,tama);
		//insertion(aux,0,tama);
